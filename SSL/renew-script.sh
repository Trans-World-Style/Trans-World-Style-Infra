...
while :; do
  if ! certbot certificates | grep -q "$DOMAIN"; then
    ...
    openssl pkcs12 -export -in ./live/$DOMAIN/fullchain.pem -inkey ./live/$DOMAIN/privkey.pem -out ./live/$DOMAIN/keystore.p12 -name ttp -CAfile ./live/$DOMAIN/chain.pem -caname root -passout pass:0000
...
# openssl pkcs12 -export -in fullchain.pem -inkey privkey.pem -out keystore.p12 -name ttp -CAfile chain.pem -caname root -passout pass:0000
